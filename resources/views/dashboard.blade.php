<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto p-4 sm:p-6 space-y-6">
        ...
        <!-- Banner: Convites de supervisão pendentes -->
        @if(isset($pendingInvitationsCount) && $pendingInvitationsCount > 0)
            <div class="bg-yellow-900/40 border border-yellow-500/50 rounded-lg px-4 py-3 flex items-center gap-3 text-yellow-200">
                ...
                <span class="text-sm flex-1">
                    Você tem <strong>{{ $pendingInvitationsCount }} {{ $pendingInvitationsCount === 1 ? 'convite de supervisão pendente' : 'convites de supervisão pendentes' }}</strong>.
                </span>
                <a href="{{ route('supervisor.invitations') }}" class="text-sm font-medium text-yellow-300 hover:text-yellow-100 underline whitespace-nowrap">
                    Ver convites →
                </a>
            </div>
        @endif
